...
def gen_html(m, fd, td):
    ...
    for n, c in [('道琼斯','red-600'),('标普500','slate-600'),('纳斯达克100','green-600'),('德国DAX','red-600'),('WTI原油','green-600'),('USDCNH','slate-700')]:
        ...
        mc_parts.append(f'<div class="bg-white rounded-xl p-4 border border-slate-200"><p class="text-xs text-slate-500 mb-1">{n}</p><p class="text-lg font-bold text-{c}">{sg}{chg}%</p></div>')
    mc_s = ''.join(mc_parts)
    ...
    for r in fd:
        ...
        strat = "高溢价卖出" if r["premium"]>15 else "底仓套利" if r["premium"]>8 else "LOF申购" if r["premium"]>3 and r["type"]=="LOF" else "折价买入" if r["premium"]<0 else "关注" if r["premium"]>3 else "持有"
        tr_parts.append(f'<tr class="hover:bg-slate-50"><td class="py-3 px-2 font-mono text-slate-900">{r["code"]}</td><td class="py-3 px-2 font-medium text-slate-900">{r["name"]}</td><td class="py-3 px-2"><span class="tag {"tag-etf" if r["type"]=="ETF" else "tag-lof"}">{r["type"]}</span></td><td class="py-3 px-2 text-slate-600">{r["track"]}</td><td class="py-3 px-2 text-right font-mono">{r["close"]:.3f}</td><td class="py-3 px-2 text-right font-mono {ac}">{acs}{r["a_chg"]:.2f}%</td><td class="py-3 px-2 text-right font-mono text-slate-600">{r["latest_nav"]:.4f}</td><td class="py-3 px-2 text-right"><span class="inline-block px-2 py-0.5 rounded text-sm font-bold {pb} {pc}">+{r["premium"]:.2f}%</span></td><td class="py-3 px-2"><span class="tag {st_c}">{s}</span></td><td class="py-3 px-2 text-xs font-medium">{strat}</td></tr>')
    tr_s = ''.join(tr_parts)
    
    # Fund data as JSON for JS
    fund_json = json.dumps(fd, ensure_ascii=False)
    
    # JS code as plain string (NOT f-string) to avoid Python/JS syntax conflicts
    js_code = r'''
const fundData = ''' + fund_json + r''';
const statusMap = {"501225":"暂停申购","501018":"暂停申购","159509":"多次暂停","161226":"限额100元","159696":"7/6恢复","513310":"曾暂停"};
function getStatus(c) { return statusMap[c] || "需确认"; }
function premClass(p) { if(p>15) return "premium-high"; if(p>5) return "premium-mid"; if(p<0) return "premium-low"; return "text-slate-600"; }
function premBg(p) { if(p>15) return "bg-premium-high"; if(p>5) return "bg-premium-mid"; if(p<0) return "bg-premium-low"; return ""; }
function renderTable() {
    const tbody = document.getElementById("fund-table-body");
    tbody.innerHTML = "";
    fundData.forEach(r => {
        const s = getStatus(r.code);
        const ac = r.a_chg > 0 ? "text-red-600" : r.a_chg < 0 ? "text-green-600" : "text-slate-600";
        const acs = r.a_chg > 0 ? "+" : "";
        const pc = premClass(r.premium);
        const pb = premBg(r.premium);
        const stClass = s.includes("暂停") ? "tag-suspended" : s.includes("限额") ? "tag-limited" : "tag-open";
        const strat = r.premium > 15 ? "高溢价卖出" : r.premium > 8 ? "底仓套利" : r.premium > 3 && r.type === "LOF" ? "LOF申购" : r.premium < 0 ? "折价买入" : r.premium > 3 ? "关注" : "持有";
        const tr = document.createElement("tr");
        tr.className = "hover:bg-slate-50";
        tr.innerHTML = `<td class="py-3 px-2 font-mono text-slate-900">${r.code}</td>
<td class="py-3 px-2 font-medium text-slate-900">${r.name}</td>
<td class="py-3 px-2"><span class="tag ${r.type === "ETF" ? "tag-etf" : "tag-lof"}">${r.type}</span></td>
<td class="py-3 px-2 text-slate-600">${r.track}</td>
<td class="py-3 px-2 text-right font-mono">${r.close.toFixed(3)}</td>
<td class="py-3 px-2 text-right font-mono ${ac}">${acs}${r.a_chg.toFixed(2)}%</td>
<td class="py-3 px-2 text-right font-mono text-slate-600">${r.latest_nav.toFixed(4)}</td>
<td class="py-3 px-2 text-right"><span class="inline-block px-2 py-0.5 rounded text-sm font-bold ${pb} ${pc}">${r.premium > 0 ? "+" : ""}${r.premium.toFixed(2)}%</span></td>
<td class="py-3 px-2"><span class="tag ${stClass}">${s}</span></td>
<td class="py-3 px-2 text-xs font-medium">${strat}</td>`;
        tbody.appendChild(tr);
    });
}
function renderChart() {
    const myChart = echarts.init(document.getElementById("chart-premium"));
    const data = [...fundData].sort((a, b) => b.premium - a.premium);
    const option = {
        tooltip: {
            trigger: "axis",
            formatter: function(params) {
                ...
                var s = "";
                if (d.premium > 0) s = "+";
                return "<div><b>" + d.name + "</b>(" + d.code + ")<br>收盘价:" + d.close + "<br>净值:" + d.latest_nav + "<br><b>溢价:" + s + d.premium + "%</b></div>";
            }
        },
        grid: { left: "3%", right: "4%", bottom: "15%", top: "10%", containLabel: true },
        xAxis: { type: "category", data: data.map(d => d.name), axisLabel: { rotate: 45, fontSize: 11, color: "#64748b" } },
        yAxis: { type: "value", name: "溢价率(%)", axisLabel: { formatter: "{value}%", color: "#64748b" }, splitLine: { lineStyle: { color: "#f1f5f9" } } },
        series: [{
            type: "bar",
            data: data.map(d => ({
                value: d.premium,
                itemStyle: { color: d.premium > 15 ? "#dc2626" : d.premium > 5 ? "#ea580c" : d.premium < 0 ? "#16a34a" : "#3b82f6" },
                name: d.name, code: d.code, close: d.close, latest_nav: d.latest_nav
            })),
            barWidth: "60%",
...
'''
    ...
    # Build HTML using string concatenation (not f-string for the JS parts)
    html = '<!DOCTYPE html>\n<html lang="zh-CN">\n<head>\n<meta charset="UTF-8">\n<meta name="viewport" content="width=device-width,initial-scale=1.0">\n<title>QDII ETF/LOF 套利分析看板</title>\n<script src="https://cdn.tailwindcss.com"></script>\n<script src="https://cdn.jsdelivr.net/npm/echarts@5.4.3/dist/echarts.min.js"></script>\n<style>\nbody{font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",Roboto,"Helvetica Neue",Arial,"Noto Sans SC",sans-serif}\n.glass{background:rgba(255,255,255,0.85);backdrop-filter:blur(12px)}\n.card-hover{transition:all 0.2s ease}\n.card-hover:hover{transform:translateY(-2px);box-shadow:0 10px 25px -5px rgba(0,0,0,0.1)}\n.premium-high{color:#dc2626}\n.premium-mid{color:#ea580c}\n.premium-low{color:#16a34a}\n.bg-premium-high{background:#fef2f2;border:1px solid #fecaca}\n.bg-premium-mid{background:#fff7ed;border:1px solid #fed7aa}\n.bg-premium-low{background:#f0fdf4;border:1px solid #bbf7d0}\n.tag{font-size:0.75rem;padding:0.125rem 0.5rem;border-radius:9999px;font-weight:500}\n.tag-etf{background:#eff6ff;color:#1e40af;border:1px solid #dbeafe}\n.tag-lof{background:#f5f3ff;color:#5b21b6;border:1px solid #e9d5ff}\n.tag-suspended{background:#fef2f2;color:#991b1b;border:1px solid #fecaca}\n.tag-limited{background:#fff7ed;color:#9a3412;border:1px solid #fed7aa}\n.tag-open{background:#f0fdf4;color:#166534;border:1px solid #bbf7d0}\n</style>\n</head>\n<body class="bg-slate-50 text-slate-800 min-h-screen">\n'
    
    html += f'<header class="sticky top-0 z-50 glass border-b border-slate-200"><div class="max-w-7xl mx-auto px-4 py-4 flex justify-between"><div class="flex gap-3"><div class="w-10 h-10 bg-blue-600 rounded-lg flex items-center justify-center text-white font-bold">Q</div><div><h1 class="text-xl font-bold text-slate-900">QDII ETF/LOF 套利分析看板</h1><p class="text-xs text-slate-500">基于底层持仓与海外市场涨跌幅估算</p></div></div><div class="text-right"><p class="text-sm font-semibold text-slate-900">{td} {dn}</p><p class="text-xs text-slate-500">更新 {now.strftime("%H:%M")}</p></div></div></header>\n'
    
    html += '<main class="max-w-7xl mx-auto px-4 py-6 space-y-6">\n'
    html += f'<section><div class="flex justify-between mb-3"><h2 class="text-lg font-bold text-slate-900">海外市场概况</h2></div><div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">{mc_s}</div></section>\n'
    html += f'<section><h2 class="text-lg font-bold text-slate-900 mb-3">重点套利机会</h2><div class="grid grid-cols-1 md:grid-cols-3 gap-4">{tc_s}</div></section>\n'
    html += '<section class="bg-white rounded-xl border border-slate-200 p-5"><h2 class="text-lg font-bold text-slate-900 mb-4">估算溢价率分布</h2><div id="chart-premium" style="width:100%;height:420px"></div></section>\n'
    html += '''<section class="grid grid-cols-1 lg:grid-cols-3 gap-4">
...
'''
    html += f'<section class="bg-white rounded-xl border border-slate-200 p-5"><div class="flex justify-between mb-4"><h2 class="text-lg font-bold text-slate-900">完整基金数据</h2></div><div class="overflow-x-auto"><table class="w-full text-sm"><thead><tr class="border-b border-slate-200 text-slate-500 text-xs uppercase"><th class="text-left py-3 px-2">代码</th><th class="text-left py-3 px-2">名称</th><th class="text-left py-3 px-2">类型</th><th class="text-left py-3 px-2">跟踪标的</th><th class="text-right py-3 px-2">收盘价</th><th class="text-right py-3 px-2">A股涨跌</th><th class="text-right py-3 px-2">最新净值</th><th class="text-right py-3 px-2">估算溢价率</th><th class="text-left py-3 px-2">申购状态</th><th class="text-left py-3 px-2">策略</th></tr></thead><tbody class="divide-y divide-slate-100" id="fund-table-body">{tr_s}</tbody></table></div></section>\n'
    html += '''<section class="bg-amber-50 rounded-xl border border-amber-200 p-5"><h2 class="text-lg font-bold text-amber-900 mb-3">风险提示</h2><div class="grid grid-cols-1 md:grid-cols-2 gap-3 text-sm text-amber-800"><div class="space-y-1"><p><strong>T+2时间差：</strong>LOF申购T+2日到账，期间美股可能大幅波动，侵蚀套利空间。</p><p><strong>外汇额度限制：</strong>501225、501018等已暂停申购；多数LOF限额100元/日。</p><p><strong>交易所停牌：</strong>159509、513310等高溢价品种可能盘中临时停牌。</p></div><div class="space-y-1"><p><strong>汇率侵蚀：</strong>人民币升值持续侵蚀美元资产回报。</p><p><strong>流动性风险：</strong>501225日成交仅约4500万，大额卖出可能冲击价格。</p><p><strong>数据偏差：</strong>估算净值基于公开数据，实际基金公司公布净值可能略有差异。</p></div></div><p class="text-xs text-amber-700 mt-3 pt-3 border-t border-amber-200">免责声明：本看板基于公开市场数据估算，不构成投资建议。</p></section>
</main>
...
def main():
    ...
    for f in FUND_LIST:
        ...
        fd.append({'code': c, 'name': f['name'], 'track': f['track'], 'type': f['type'],
                   'close': round(close, 3), 'a_chg': round(ac, 2),
                   'nav_date': n[0]['date'], 'latest_nav': round(ln, 4),
                   'est_nav': round(ln, 4), 'premium': round(p, 2)})
